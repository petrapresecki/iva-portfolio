...
function NavPill({
  ...
  return (
    <button
      onClick={onClick}
      className={`nav-pill group relative !cursor-pointer overflow-hidden rounded-full border px-5 py-1.5 font-display text-[14px] font-medium transition-shadow duration-300 md:px-7 md:py-2 md:text-[20px] ${
        isDark
...
function Header({ activeTab, onTabChange, variant = 'dark' }: HeaderProps) {
  ...
  return (
    <header className="flex items-center justify-between px-6 py-6 md:px-[60px] md:py-[30px]">
      {/* Name / logo */}
      <motion.button
